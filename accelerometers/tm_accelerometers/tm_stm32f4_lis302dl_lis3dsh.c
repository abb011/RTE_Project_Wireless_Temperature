...
void TM_LIS302DL_LIS3DSH_INT_ReadSPI(uint8_t* data, uint8_t addr, uint8_t count) {
	/* Start SPI transmission by setting the CS line low*/
	LIS302DL_LIS3DSH_CS_LOW;
	/* Add read bit */
	addr |= 0x80;
	...
	/* Send address using the TM_SPI_Send function */
	TM_SPI_Send(LIS302DL_LIS3DSH_SPI, addr);
	/* Receive data using the TM_SPI_ReadMulti function*/
	int8_t dummy = 0;
	TM_SPI_ReadMulti(LIS302DL_LIS3DSH_SPI, data, dummy, count);	
	
	/* Stop SPI transmission by setting the CS line high*/
	LIS302DL_LIS3DSH_CS_HIGH;
}
...
void TM_LIS3DSH_INT_ReadAxes(TM_LIS302DL_LIS3DSH_t *Axes_Data) {	
	...
	// Read data from accelerometer over SPI using TM_LIS302DL_LIS3DSH_INT_ReadSPI function 
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)&(d[0]), LIS3DSH_OUT_X_L_ADDR, count);
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)&(d[1]), LIS3DSH_OUT_X_H_ADDR, count);
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)&(d[2]), LIS3DSH_OUT_Y_L_ADDR, count);
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)&(d[3]), LIS3DSH_OUT_Y_H_ADDR, count);
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)&(d[4]), LIS3DSH_OUT_Z_L_ADDR, count);
	TM_LIS302DL_LIS3DSH_INT_ReadSPI((uint8_t*)&(d[5]), LIS3DSH_OUT_Z_H_ADDR, count);
	
	// Put received data into Axes_Data 
	Axes_Data->X = (int16_t)((d[1] << 8) + d[0]) * TM_LIS3DSH_INT_Sensitivity;
	Axes_Data->Y = (int16_t)((d[3] << 8) + d[2]) * TM_LIS3DSH_INT_Sensitivity;
	Axes_Data->Z = (int16_t)((d[5] << 8) + d[4]) * TM_LIS3DSH_INT_Sensitivity;
}
